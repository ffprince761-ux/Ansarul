...
import { API_URL, getAppSettings } from '../../services/api';
...
const ProfileScreen = ({ navigation }) => {
  ...
  useEffect(() => {
    getAppSettings().then(res => {
      if (res?.success && res.settings?.app_version) {
        setAppVersion(res.settings.app_version);
      }
    }).catch(() => {});
  }, []);
  ...
  useEffect(() => {
    if (user) {
      setBusinessName(user.businessName || user.business_name || '');
