...
function formatDeviceInfo(deviceInfo) {
    ...
    const device = deviceInfo.toString().toLowerCase().trim();
    
    // Android detection
    if (device.includes('android')) {
        const match = device.match(/android\s*(\d+)?/);
        const apiLevel = match && match[1] ? parseInt(match[1]) : null;
        ...
        if (apiLevel && apiToVersion[apiLevel]) {
            return 'Android ' + apiToVersion[apiLevel];
        }
        return apiLevel ? 'Android ' + apiLevel : 'Android';
    }
    
    // iOS detection
    if (device.includes('ios') || device.includes('iphone') || device.includes('ipad')) {
        const match = device.match(/(?:ios|iphone|ipad)\s*(\d+(?:\.\d+)?)?/);
        const version = match && match[1] ? match[1] : null;
        return version ? 'iOS ' + version : 'iOS';
    }
    
    if (device.includes('web')) return 'Web';
    if (device.includes('windows')) return 'Windows';
    if (device.includes('mac') || device.includes('darwin')) return 'macOS';
    if (device.includes('linux')) return 'Linux';
    
    return deviceInfo.charAt(0).toUpperCase() + deviceInfo.slice(1);
}
...
function renderUserActivity(users) {
    ...
    (users.active_users || []).forEach(u => {
        ...
        const device = formatDeviceInfo(u.device_info);
